      ...
      while (node = events.pop()) {
        ...
        while ((node = node.next) !== tail) {
          if (window.CATCH_ERROR){
            try{node.callback.apply(node.context || this, args);}catch(e){proc_error(e);}
          }else{
    ...
    delegateEvents: function(events) {
      ...
      for (var key in events) {
        ...
        var eventName = match[1], selector = match[2];
				...
				if (eventName == "click")
					var _method = __.prevent(method);
				else
					var _method = method;
				
        _method = _.bind(_method, this);
        eventName += '.delegateEvents' + this.cid;
        if (selector === '') {
          this.$el.bind(eventName, _method);
        } else {
          this.$el.delegate(selector, eventName, _method);
        }
      }
    },
